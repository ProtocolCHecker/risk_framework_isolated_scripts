...
def analyze_compound_market(collateral_address, chain_name, chain_config):
    """Analyze Compound v3 market for a specific collateral"""
    print(f"\n{'='*70}")
    ...
    try:
        ...
        for market_name, comet_address in chain_config['markets'].items():
            ...
            for i in range(0, len(results), 2):
                borrow_balance = w3.codec.decode(['uint256'], results[i])[0] / (10 ** base_decimals)
                collateral_balance = w3.codec.decode(['uint128'], results[i+1])[0] / (10 ** collateral_decimals)
                ...
                if hf < 1.0:
                    risk_buckets["Critical (HF < 1.0)"].append((borrowers[i//2], hf, borrow_balance))
                    debt_at_risk += borrow_balance
                elif hf < 1.05:
                    risk_buckets["High Risk (1.0 ≤ HF < 1.05)"].append((borrowers[i//2], hf, borrow_balance))
                    debt_at_risk += borrow_balance
                elif hf < 1.1:
                    risk_buckets["At Risk (1.05 ≤ HF < 1.1)"].append((borrowers[i//2], hf, borrow_balance))
                    debt_at_risk += borrow_balance
                elif hf < 1.25:
                    risk_buckets["Moderate (1.1 ≤ HF < 1.25)"].append((borrowers[i//2], hf, borrow_balance))
                else:
                    risk_buckets["Safe (HF ≥ 1.25)"].append((borrowers[i//2], hf, borrow_balance))
...
if __name__ == "__main__":
    ...
    for chain_name, config in MARKETS.items():
        analyze_compound_market(collateral_token, chain_name, config)
        print()