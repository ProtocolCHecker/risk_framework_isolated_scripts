"""
High Frequency Lambda Handler (30 min interval).
...
from core.dispatcher import dispatch_high
from notifications import process_pending_alerts_telegram


def handler(event, context):
    """
    AWS Lambda handler for high frequency metrics.
    ...
    try:
        # Dispatch high frequency metrics
        print(f"[{start_time.isoformat()}] Starting high frequency metrics dispatch...")
        dispatch_result = dispatch_high()
...
if __name__ == "__main__":
    print("Testing high frequency handler locally...")
    result = handler({}, None)
