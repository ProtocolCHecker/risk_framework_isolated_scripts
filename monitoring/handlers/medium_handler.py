"""
Medium Frequency Lambda Handler (6 hour interval).
...
from core.dispatcher import dispatch_medium
from notifications import process_pending_alerts_telegram


def handler(event, context):
    """
    AWS Lambda handler for medium frequency metrics.
    ...
    try:
        # Dispatch medium frequency metrics
        print(f"[{start_time.isoformat()}] Starting medium frequency metrics dispatch...")
        dispatch_result = dispatch_medium()
...
if __name__ == "__main__":
    print("Testing medium frequency handler locally...")
    result = handler({}, None)
