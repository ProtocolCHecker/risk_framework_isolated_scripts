"""
Daily Frequency Lambda Handler (24 hour interval).
...
from core.dispatcher import dispatch_daily
from notifications import process_pending_alerts_telegram


def handler(event, context):
    """
    AWS Lambda handler for daily frequency metrics.
    ...
    try:
        # Dispatch daily metrics
        print(f"[{start_time.isoformat()}] Starting daily metrics dispatch...")
        dispatch_result = dispatch_daily()
...
if __name__ == "__main__":
    print("Testing daily handler locally...")
    result = handler({}, None)
