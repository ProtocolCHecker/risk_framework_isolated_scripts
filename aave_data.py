...
def analyze_aave_market(token_address, chain_name, chain_config):
    """Complete AAVE market analysis for any token. Returns dict with all metrics."""
    print(f"\n{'='*70}")
    ...
    try:
        ...
        # Store market overview in result
        ...
        if not suppliers or not borrowers:
            ...
            result["rlr"] = {"error": "Insufficient holder data"}
            result["clr"] = {"error": "Insufficient holder data"}
        ...
        # Store RLR in result
        result["rlr"] = {
            "loopers_count": len(looper_details),
            "looped_borrow": looped_borrow,
            "rlr_supply_based": rlr_supply,
            "rlr_borrow_based": rlr_borrow,
            "leverage_stats": {
                "average": leverage_avg,
                "max": leverage_max,
                "min": leverage_min
            },
            "top_loopers": looper_details[:10]
        }
        ...
        for i, res in enumerate(results):
            decoded = w3.codec.decode(['uint256', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'], res)
            ...
            if hf < 1.0:
                risk_buckets["critical"].append({"address": borrower_list[i], "health_factor": hf, "debt_usd": debt_usd})
                debt_at_risk += debt_usd
            elif hf < 1.05:
                risk_buckets["high_risk"].append({"address": borrower_list[i], "health_factor": hf, "debt_usd": debt_usd})
                debt_at_risk += debt_usd
            elif hf < 1.1:
                risk_buckets["at_risk"].append({"address": borrower_list[i], "health_factor": hf, "debt_usd": debt_usd})
                debt_at_risk += debt_usd
            elif hf < 1.25:
                risk_buckets["moderate"].append({"address": borrower_list[i], "health_factor": hf, "debt_usd": debt_usd})
            else:
                risk_buckets["safe"].append({"address": borrower_list[i], "health_factor": hf, "debt_usd": debt_usd})
        ...
        result["clr"] = {
            ...
            "debt_analyzed_usd": total_debt_analyzed,
...
if __name__ == "__main__":
    ...
    for chain_name, config in CHAINS.items():
        result = analyze_aave_market(token_address, chain_name, config)
        if result:
